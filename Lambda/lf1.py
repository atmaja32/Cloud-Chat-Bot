...
def greet_user(intent_request):
    ...
    return close(
        ...
        'Fulfilled',
        {
            ...
            'content': 'Howdy, how may i be of assistance to you?'
        }
    )
...
def respond_to_thanks(intent_request):
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Your welcome!have an amazing day eat well'
        }
    )
...
def suggest_restaurant(intent_request):
    ...
    if intent_request['invocationSource'] == 'DialogCodeHook':
        ...
        if party_int is not None and (party_int < 1 or party_int > 20):
            ...
            invalidSlot = 'phone'
            invalidMessage = 'not a valid phone number please a put a correct one'
        elif email is not None and '@' not in email:
            invalidSlot = 'email'
            invalidMessage = 'renter the eamil, its not right'
    ...
    client.send_message(
        QueueUrl='https://sqs.us-east-1.amazonaws.com/411297634259/deeptisqs', 
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'You\'re good to go. Expect my suggestions shortly on {} and {}! Have an awesome day.'.format(phone, email)
        }
    )
